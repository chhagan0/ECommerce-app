...
import com.example.relix.*
import com.example.relix.isEmailValid
import com.example.relix.isPhoneValid
...
class AuthViewModel(application: Application) : AndroidViewModel(application) {
	...
	private val _userData = MutableLiveData<com.vishalgaur.shoppingapp.data.UserData>()
	val userData: LiveData<com.vishalgaur.shoppingapp.data.UserData> get() = _userData
				...
				if (!isAccepted) {
					...
					when (err) {
						ERR_INIT -> {
							...
							val newData =
								com.vishalgaur.shoppingapp.data.UserData(
									uId,
									name.trim(),
									"+91" + mobile.trim(),
									email.trim(),
	...
	private fun checkUniqueUser(uData: com.vishalgaur.shoppingapp.data.UserData) {
		viewModelScope.launch {
